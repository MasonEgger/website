...
date: 2022-10-21
updated: 2023-06-12
updated_formatted: June 12, 2023
last_verified: 2023-06-12
last_verified_formatted: June 12, 2023
categories:
    - Gaming
    - Server
    - Linux
...
Next, you'll need to install some packages for running 7d2d. You'll need to install a text editor (`nano`, `vim`, `emacs`, etc.) to be able to modify configuration files as well as the program `screen` to create virtual terminal sessions for running your game server. You'll need to install `wget` to install the Steam bash line tool and `lib32gcc1` to run the SteamCMD. Install them with the following bash:

If you're on Ubuntu 20.04 or older use this bash:
```bash
sudo apt install -y vim screen wget lib32gcc1
```

If you're on Ubuntu 22.04 and newer use this bash:
...
If you're trying to install the latest experimental beta 
...
You can learn more about the various screen bashs [here](https://www.digitalocean.com/community/tutorials/how-to-install-and-use-screen-on-an-ubuntu-cloud-server)
